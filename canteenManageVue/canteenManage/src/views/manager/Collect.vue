...
const handleSelectionChange = (rows) => {
  data.ids = rows.map(item => item.id)// 将选中的收藏记录id赋值给ids数组
}
//分页查询收藏记录信息的函数
const load = () => {
  request.get('/collect/selectPage', {
    ...
    if (res.code === '200') {
      data.tableData = res.data?.list || []// 将从后端获取到的收藏记录信息列表赋值给表格数据对象  
...
const del = (row) => {//传整行数据，否则可能无法请求到id
  ElMessageBox.confirm('是否删除该收藏记录信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
    request.delete(`/collect/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  if (!data.ids.length) {
    ElMessage.warning('请选择要删除的收藏记录信息')
    return
  }
  ElMessageBox.confirm('是否删除该收藏记录信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
