  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 10px;">
      <el-table stripe :data="data.tableData" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="dish.img" label="菜品图片" >
          <template v-slot="scope">
            <el-image :src="scope.row.dish.img" :preview-src-list="[scope.row.dish.img]" fit="cover"
...
<script setup>
import { reactive} from 'vue'// 从 Vue 的composition中导入 reactive 函数，用于创建响应式的对象
...
const data = reactive({
  ...
  total: 0,// 总条数，初始设置为0
  studentName: null,// 查询条件:标题，初始设置为null
  ids: [],// 选中的id，初始设置为空数组
  rules: {
    title: [
...
const handleSelectionChange = (rows) => {
  data.ids = rows.map(item => item.id)// 将选中的点赞记录id赋值给ids数组
}
//分页查询点赞记录信息的函数
const load = () => {
  request.get('/praise/selectPage', {
    params: {
    ...
    if (res.code === '200') {
      data.tableData = res.data?.list || []// 将从后端获取到的点赞记录信息列表赋值给表格数据对象  
...
const del = (row) => {//传整行数据，否则可能无法请求到id
  ElMessageBox.confirm('是否删除该点赞记录信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
    request.delete(`/praise/delete/${row.id}`).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  if (!data.ids.length) {
    ElMessage.warning('请选择要删除的点赞记录信息')
    return
  }
  ElMessageBox.confirm('是否删除该点赞记录信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
    request.delete(`/praise/delete/batch`, { data: data.ids }).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        load()
      } else {
