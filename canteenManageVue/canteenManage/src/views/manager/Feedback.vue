<template>
      ...
      <el-table stripe :data="data.tableData" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" width="180" >
          <template v-slot="scope">
            <el-button type="success" plain @click="handleEdit(scope.row)">回复</el-button>
  ...
  <!----------------- 回复反馈意见的弹窗 ------------------>
  <el-dialog title="反馈回复" v-model="data.formVisible" width="30%">
...
const handleEdit = (row) => {
  data.form = JSON.parse(JSON.stringify(row))// 将要编辑的反馈意见信息赋值给表单数据对象
  data.formVisible = true// 将表单的可见状态设置为true，显示表单
}
//处理选中行变化的函数 rows --> 当前选中的行数据数组
const handleSelectionChange = (rows) => {
  data.ids = rows.map(item => item.id)// 将选中的反馈意见id赋值给ids数组
}
//分页查询反馈意见信息的函数
const load = () => {
  request.get('/feedback/selectPage', {
    ...
    if (res.code === '200') {
      data.tableData = res.data?.list || []// 将从后端获取到的反馈意见信息列表赋值给表格数据对象  
...
const del = (row) => {//传整行数据，否则可能无法请求到id
  ElMessageBox.confirm('是否删除该反馈意见信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
...
const delBatch = () => {
  if (!data.ids.length) {
    ElMessage.warning('请选择要删除的反馈意见信息')
    return
  }
  ElMessageBox.confirm('是否删除该反馈意见信息?（删除后将无法恢复）', '删除确认', { type: 'warning' }).then(res => {
